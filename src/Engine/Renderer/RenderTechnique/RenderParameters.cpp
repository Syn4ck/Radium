...
namespace Ra
{
    namespace Engine
    {
        ...
        void RenderParameters::addParameter( const std::string& name, Texture* tex )
        {
        ...
        void RenderParameters::updateParameter( const std::string& name, int    value )
        {
            m_intParamsVector[name] = IntParameter( name, value );
        }

        void RenderParameters::updateParameter( const std::string& name, uint   value )
        {
            m_uintParamsVector[name] = UIntParameter( name, value );
        }

        void RenderParameters::updateParameter( const std::string& name, Scalar value )
        {
            m_scalarParamsVector[name] = ScalarParameter( name, value );
        }

        void RenderParameters::updateParameter( const std::string& name, const Core::Vector2& value )
        {
            m_vec2ParamsVector[name] = Vec2Parameter( name, value );
        }

        void RenderParameters::updateParameter( const std::string& name, const Core::Vector3& value )
        {
            m_vec3ParamsVector[name] = Vec3Parameter( name, value );
        }

        void RenderParameters::updateParameter( const std::string& name, const Core::Vector4& value )
        {
            m_vec4ParamsVector[name] = Vec4Parameter( name, value );
        }

        void RenderParameters::updateParameter( const std::string& name, const Core::Matrix2& value )
        {
            m_mat2ParamsVector[name] = Mat2Parameter( name, value );
        }

        void RenderParameters::updateParameter( const std::string& name, const Core::Matrix3& value )
        {
            m_mat3ParamsVector[name] = Mat3Parameter( name, value );
        }

        void RenderParameters::updateParameter( const std::string& name, const Core::Matrix4& value )
        {
            m_mat4ParamsVector[name] = Mat4Parameter( name, value );
        }
        ...
        Texture* RenderParameters::getTexParameter( const std::string& name )
        {
            return m_texParamsVector[name].m_texture;
        }

        int RenderParameters::getIntParameter ( const std::string& name )
        {
            return m_intParamsVector[name].m_value;
        }

        uint RenderParameters::getUintParameter ( const std::string& name )
        {
            return m_uintParamsVector[name].m_value;
        }

        Scalar RenderParameters::getScalarParameter( const std::string& name )
        {
            return m_scalarParamsVector[name].m_value;
        }

        Core::Vector2& RenderParameters::getVec2Parameter( const std::string& name )
        {
            return m_vec2ParamsVector[name].m_value;
        }

        Core::Vector3& RenderParameters::getVec3Parameter( const std::string& name )
        {
            return m_vec3ParamsVector[name].m_value;
        }

        Core::Vector4& RenderParameters::getVec4Parameter( const std::string& name )
        {
            return m_vec4ParamsVector[name].m_value;
        }

        Core::Matrix2& RenderParameters::getMat2Parameter( const std::string& name )
        {
            return m_mat2ParamsVector[name].m_value;
        }

        Core::Matrix3& RenderParameters::getMat3Parameter( const std::string& name )
        {
            return m_mat3ParamsVector[name].m_value;
        }

        Core::Matrix4& RenderParameters::getMat4Parameter( const std::string& name )
        {
