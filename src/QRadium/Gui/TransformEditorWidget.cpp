#include <QRadium/Gui/TransformEditorWidget.hpp>
#include <QVBoxLayout>
#include <QRadium/Gui/VectorEditor.hpp>
#include <QRadium/Gui/RotationEditor.hpp>
...
namespace QRadium
{
    ...
    void TransformEditorWidget::updateValues()
    {
        if(m_currentEdit)
        {
            ...
            Ra::Core::AlignedStdVector<Ra::Engine::EditableProperty> props;
            m_currentEdit->getProperties(props);
            for (uint i = 0; i < props.size(); ++i)
            {
                if (props[i].type == Ra::Engine::EditableProperty::TRANSFORM)
                {
                    m_transform = props[i];
                    break;
                }
            ...
            {
                const Ra::Engine::EditablePrimitive& prim = m_transform.primitives[p].primitive;
                switch (prim.getType())
                {
    ...
    void TransformEditorWidget::onChangedPosition(const Ra::Core::Vector3& v, uint id)
    {
    ...
    void TransformEditorWidget::onChangedRotation(const Ra::Core::Quaternion& q, uint id)
    {
        CORE_ASSERT(m_currentEdit, "Nothing to edit");
        CORE_ASSERT(sender() == m_widgets[id], "Sender error");
    ...
    void TransformEditorWidget::setEditable(Ra::Engine::EditableInterface* edit)
    {
