...
namespace QRadium
{
    ...
    void MaterialEditor::onKdColorChanged( int )
    {
        ...
        if ( m_renderObject )
        {
            Ra::Core::Color c( kdR->value() / 255.f, kdG->value() / 255.f, kdB->value() / 255.f, 1.0 );
    ...
    void MaterialEditor::onKsColorChanged( int )
    {
        ...
        if ( m_renderObject )
        {
            Ra::Core::Color c( ksR->value() / 255.f, ksG->value() / 255.f, ksB->value() / 255.f, 1.0 );
    ...
    void MaterialEditor::newKdColor( const QColor& color )
    {
        ...
        if ( m_renderObject )
        {
            Ra::Core::Color c( color.redF(), color.greenF(), color.blueF(), 1.0 );
            m_renderObject->getRenderTechnique()->material->setKd( c );
        }
    }
    ...
    void MaterialEditor::changeRenderObject( Ra::Core::Index roIdx )
    {
        if ( !m_visible )
        {
            return;
    ...
    void MaterialEditor::updateMaterialViz( const Ra::Engine::Material* material )
    {
        const Ra::Core::Color ks = material->getKs();
        const Ra::Core::Color kd = material->getKd();
